...
export async function createSPAPDF(dealId) {
  ...
  try {
    ...
    const dealData = await BX24.callMethod('crm.deal.get', { ID: dealId });
    const fields = dealData.data().fields || {};
    ...
    // Payment Schedule
    currentY = addPaymentSchedule(doc, currentY, pageWidth, margin, paymentPlan);

    // Property Details
    currentY = addPropertyDetails(doc, currentY, pageWidth, margin, fields);

    // Terms & Conditions (Fixed)
    currentY = addTermsAndConditions(doc, currentY, pageWidth, margin, pageHeight);
...
function addTitle(doc, y, pageWidth) {
  ...
  doc.setFontSize(18);
  doc.text('SALE AND PURCHASE AGREEMENT', pageWidth / 2, y + 10, { align: 'center' });
  ...
  doc.text(`This agreement is made on this day of ${today}`, pageWidth / 2, y + 20, { align: 'center' });
...
function addHeader(doc, y, pageWidth, margin, fields, buyerData) {
  ...
  const projectLocation = fields.UF_CRM_PROJECT_LOCATION || 'Plot # 04, Commercial Mall, Road # 35 Mini, Extension-II Sector F DHA Phase 1, Rawalpindi';
...
async function fetchPaymentSchedule(dealFields) {
  try {
    ...
    const startDate = new Date(dealFields.UF_CRM_PAYMENT_START_DATE || new Date());

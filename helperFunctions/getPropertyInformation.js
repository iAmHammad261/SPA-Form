...
export const getPropertyInformation = async (dealId) => {
  ...
  var productRows = await callBX24Method("crm.deal.productrows.get", {
    id: dealId,
  });
  ...
  productInformation = await callBX24Method("crm.product.get", {
    id: productId,
  });
  ...
  var {
    PROPERTY_173: projectFieldValue,
    NAME: propertyNameValue,
    PROPERTY_139: propertyTypeValue,
    PROPERTY_113: grossAreaValue,
    PROPERTY_115: basicRateValue,
    PROPERTY_135: floorValue,
    PROPERTY_149: netAreaValue,
  } = productInformation;
  ...
  var productData = {
    ...
    PROJECT: await getListPropertyValue(projectFieldValue["value"]).catch(
      () => projectFieldValue["value"],
    ),
    PROPERTY_TYPE: await getListPropertyValue(propertyTypeValue["value"]).catch(
      () => propertyTypeValue["value"],
    ),
    PROPERTY_FLOOR: await getListPropertyValue(floorValue["value"]).catch(
      () => floorValue["value"],
    ),
